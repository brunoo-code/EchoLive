...
export default function ProfilePopover({ profile, nickname, avatarUrl, onStatusChange, onEditProfile, onOpenSettings, onClose }) {
  ...
  return <section className="profile-popover" role="dialog" aria-label="Menu do perfil">
    <div className="profile-popover-header">
      <div className="profile-popover-avatar">{avatarUrl ? <img src={avatarUrl} alt="" /> : displayName.slice(0, 1).toUpperCase()}<UserStatusBadge status={currentStatus} /></div>
    ...
    <div className="profile-status-options" aria-label="Status do perfil">
      <button type="button" className={currentStatus === "online" ? "is-selected" : ""} onClick={() => onStatusChange("online")}><UserStatusBadge status="online" />Online</button>
      <button type="button" className={currentStatus === "dnd" ? "is-selected" : ""} onClick={() => onStatusChange("dnd")}><UserStatusBadge status="dnd" />Nao perturbe</button>
    </div>
