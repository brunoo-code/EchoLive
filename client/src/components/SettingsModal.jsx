      ...
      <div className="settings-layout">
        ...
        <div className="settings-content">
          {active === "profile" && <section className="settings-section">
            <h3>Perfil</h3>
            <div className="profile-edit-preview"><div className="profile-preview">{draft.avatarUrl ? <img src={draft.avatarUrl} alt="" /> : displayName.slice(0, 1).toUpperCase()}<UserStatusBadge status={draft.status} /></div><div><strong>{displayName}</strong><span>@{draft.nickname || "nickname"}</span><small><UserStatusBadge status={draft.status} />{shortStatus}{draft.customStatus ? ` - ${draft.customStatus}` : ""}</small></div></div>
