...
export default function SettingsModal({ initialSection = "profile", theme, onThemeChange, accentColor, onAccentChange, uiSounds, onUiSoundsChange, confirmLeaveRoom, onConfirmLeaveChange, onOpenDevices, streamPreset, onStreamPresetChange, profile, onProfileChange, onClose }) {
  ...
  return <div className="modal-backdrop" role="presentation">
    <section className="settings-modal" role="dialog" aria-modal="true" aria-labelledby="settings-title">
      <header className="settings-header"><div><p className="section-label">Preferencias</p><h2 id="settings-title">Configuracoes</h2></div><button type="button" className="icon-button" onClick={onClose} aria-label="Fechar configuracoes" title="Fechar"><Icon name="close" size={17} /></button></header>
      <div className="settings-layout">
        <nav className="settings-nav" aria-label="Secoes de configuracoes">{sections.map((section) => <button type="button" key={section} className={active === section ? "settings-nav-active" : ""} onClick={() => setActive(section)}><span className="settings-nav-icon" aria-hidden="true"><Icon name={section === "profile" ? "user" : section === "account" ? "account" : section === "voice" ? "voice" : section === "appearance" ? "palette" : "sliders"} size={16} /></span><span className="settings-nav-copy"><strong>{labels[section]}</strong><small>{descriptions[section]}</small></span></button>)}</nav>
        <div className="settings-content">
          ...
          {active === "voice" && <section className="settings-section"><h3>Voz e video</h3><p>Escolha microfone, saida de audio, camera e qualidade da transmissao.</p><button type="button" className="ghost-button" onClick={onOpenDevices}>Abrir dispositivos</button><div className="settings-subsection"><span className="settings-subsection-title">Qualidade da transmissao</span><div className="stream-preset-grid" aria-label="Qualidade da transmissao">
            {[['720p30', '720p', '30 FPS'], ['720p60', '720p', '60 FPS'], ['1080p30', '1080p', '30 FPS'], ['1080p60', '1080p', '60 FPS']].map(([value, resolution, fps]) => <button type="button" key={value} className={`stream-preset ${streamPreset === value ? "is-selected" : ""}`} onClick={() => onStreamPresetChange(value)} aria-pressed={streamPreset === value}><strong>{resolution}</strong><span>{fps}</span>{value === "1080p60" && <small>Maior uso de conexao e processamento</small>}</button>)}
