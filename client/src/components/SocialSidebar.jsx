...
export default function SocialSidebar({ activeTab, onTabChange, conversations, onlineUserIds, user, onHome, onOpenConversation }) {
  const tabs = [
  ...
  return <aside className="social-sidebar">
    ...
    <nav className="social-nav" aria-label="Navegacao social">
      {tabs.map(([id, label, icon]) => <button type="button" key={id} className={activeTab === id ? "is-active" : ""} onClick={() => onTabChange(id)}><Icon name={icon} size={16} /><span>{label}</span></button>)}
    ...
    <div className="social-sidebar-section">
      ...
      <div className="social-conversation-list">
        {conversations.map((conversation) => <button type="button" className="social-conversation-row" key={conversation.id} onClick={() => onOpenConversation(conversation.id)}><span className="social-conversation-avatar"><Avatar user={conversation.user} size={32} /><i className={onlineUserIds.has(conversation.user.id) ? "is-online" : ""} /></span><span className="social-conversation-copy"><strong>{conversation.user.displayName || conversation.user.username}</strong><small>@{conversation.user.username}</small></span>{conversation.unreadCount > 0 && <b className="social-unread-badge">{conversation.unreadCount > 9 ? "9+" : conversation.unreadCount}</b>}</button>)}
