const markVariants = [
  ["#164e63", "#f5f8fc", "#67d8e4", "#071522"],
  ["#1f5c46", "#f5f8fc", "#71d6a0", "#071b17"],
  ["#7c4a1d", "#f5f8fc", "#f5c27b", "#241509"],
  ["#762f39", "#f5f8fc", "#f5a4a9", "#241018"],
  ["#49318d", "#f5f8fc", "#a895ff", "#100c27"],
  ["#495361", "#f5f8fc", "#b8c4d1", "#16202a"]
];
...
export default function BrandMark({ size = 40, className = "", variant = null }) {
  ...
  const style = colors ? { "--brand-bg": colors[0], "--brand-shell": colors[1], "--brand-accent": colors[2], "--brand-visor": colors[3] } : defaultMarkStyle;
  const markClassName = `brand-symbol-svg ${colors ? "guest-avatar-mark" : ""} ${className}`.trim();

  return <svg className={markClassName} style={style} width={size} height={size} viewBox="0 0 48 48" role="img" aria-label="EchoLive">
    {colors && <circle cx="24" cy="24" r="23" fill="var(--brand-bg)" />}
    <path d="M21.2 6.5c.3-3.4 1.8-5.2 3.7-5 1.7.2 2.1 1.6 1.4 2.8-1.1 1.8-2.7 2.7-5.1 2.2Z" fill="var(--brand-accent, var(--echo-accent, #61dceb))" />
