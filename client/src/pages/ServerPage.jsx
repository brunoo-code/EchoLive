...
export default function ServerPage({ serverId, onNavigateHome, onNavigateSocial, onNavigateServer, onOpenAccountSettings }) {
  ...
  const serverVoice = useServerVoiceCall({ socket: serverSocket, serverId, channelId: voiceChannelId, identity: voiceIdentity, enabled: Boolean(voiceChannelId), notify, uiSounds: uiSoundsEnabled(user?.status) });
  ...
  function toggleDeafen() {
    setIsDeafened((current) => {
      const next = !current;
      playUiSound(next ? "deafen-on" : "deafen-off", uiSoundsEnabled(user?.status));
      return next;
    });
  }
  ...
  async function sendMessage(event) {
    ...
    try {
      ...
      playUiSound("message-send", uiSoundsEnabled(user?.status));
      setDraft("");
      stopServerTyping();
      setSelectedFile(null);
      setReplyingTo(null);
  ...
  function renderServerMessage(message, index) {
    ...
    return (
      <article id={`server-message-${message.id}`} className={`chat-message server-chat-message${isGrouped ? " is-grouped" : ""}`} key={message.id} data-flx="channel.message-group.message" data-message-id={message.id} data-is-group-start={!isGrouped}>
        {isGrouped ? <div className="message-avatar-placeholder server-message-time" aria-hidden="true"><time dateTime={message.createdAt}>{formatServerTime(message.createdAt)}</time></div> : <button type="button" className="message-avatar server-message-avatar" onClick={(event) => toggleProfileUser(message.sender, event.currentTarget.getBoundingClientRect())} aria-label={`Ver perfil de ${senderName}`}><UserAvatar user={message.sender} size={36} /></button>}
        <div className="message-body" data-flx="channel.message-group.message-body">
          ...
          {attachment && attachmentSource && <div className="message-attachment server-message-attachment" data-flx="channel.message-attachments">
