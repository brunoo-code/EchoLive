...
export default function ServerPage({ serverId, onNavigateHome, onNavigateSocial, onNavigateServer }) {
  ...
  return <main className="page app-shell room-page server-page">
    ...
    <section className="central-stage server-main chat-stage">
      <header className="server-main-header chat-header"><div className="channel-title"><span className="channel-kind"><Icon name={activeChannel?.type === "voice" ? "voice" : "hash"} size={17} /></span><strong>{activeChannel?.name || (showServerEmpty ? "Seus servidores" : "Selecione um canal")}</strong></div><span className="channel-subtitle">{server ? `${server.memberCount} membro${server.memberCount === 1 ? "" : "s"}` : ""}</span></header>
      ...
      {activeChannel?.type === "text" && <form className="server-composer chat-composer" onSubmit={sendMessage}>{replyingTo && <div className="server-replying"><span>Respondendo a {replyingTo.sender.displayName || replyingTo.sender.username}</span><button type="button" className="icon-button" onClick={() => setReplyingTo(null)} aria-label="Cancelar resposta"><Icon name="close" size={13} /></button></div>}<div className="composer-row"><button type="button" className="icon-button" title="Anexos indisponíveis nesta primeira camada" aria-label="Anexos indisponíveis"><Icon name="plus" size={18} /></button><input value={draft} onChange={(event) => setDraft(event.target.value)} placeholder={`Conversar em #${activeChannel.name}`} maxLength={4000} /><button type="submit" className="icon-button is-send" title="Enviar mensagem" aria-label="Enviar mensagem"><Icon name="send" size={16} /></button></div></form>}
    </section>
    <aside className="participants-panel server-members-panel"><div className="server-members-heading panel-heading"><span><span className="online-indicator" />Online</span><strong>{server?.memberCount || 0}</strong></div><div className="server-members-list online-list">{showServerLoading ? <ServerMembersLoadingState /> : members.map((member) => <button type="button" className="server-member-row online-person" key={member.id} onClick={() => setProfileUser(member)}><UserAvatar user={member} size={28} /><span className="online-person-info"><strong>{member.displayName || member.username}</strong><small>{member.role === "owner" ? "Proprietário" : member.role === "admin" ? "Administrador" : "Membro"}</small><UserBadges user={member} badges={member.badges} compact /></span></button>)}{!showServerLoading && !members.length && <div className="server-members-note"><Icon name="account" size={18} /><p>Nenhum membro disponível.</p></div>}</div></aside>
