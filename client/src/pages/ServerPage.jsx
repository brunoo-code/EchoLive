...
export default function ServerPage({ serverId, onNavigateHome, onNavigateSocial, onNavigateServer, onOpenAccountSettings }) {
  ...
  const [channelRequestBusy, setChannelRequestBusy] = useState(false);
  ...
  const memberParticipants = useMemo(() => members.map((member) => ({
    ...
    status: member.id === user?.id ? user?.status || member.status || "online" : publicPresence(member.status),
    isLocal: member.id === user?.id,
    secondaryText: member.role === "owner" ? "Proprietário" : member.role === "admin" ? "Administrador" : "Membro",
    rawUser: member
  })), [members, user?.id, user?.status]);
  ...
  const serverVoice = useServerVoiceCall({ socket: serverSocket, serverId, channelId: voiceChannelId, identity: voiceIdentity, enabled: Boolean(voiceChannelId), notify, uiSounds: uiNotificationSoundsEnabled(user?.status) });
  ...
  const serverShareParticipant = serverCallParticipants.find((participant) => participant.isScreenSharing && participant.stream);
  ...
  function toggleDeafen() {
    setIsDeafened((current) => {
      const next = !current;
      playUiSound(next ? "deafen-on" : "deafen-off", uiNotificationSoundsEnabled(user?.status));
      return next;
    });
  }
  ...
  useEffect(() => {
    if (!serverSocket || !serverId) return undefined;

    const watchServer = () => serverSocket.emit("server:watch", { serverId });
    const handleChannelCreated = ({ serverId: eventServerId, channel } = {}) => {
      if (eventServerId !== serverId || !channel?.id) return;
      setServer((current) => current ? { ...current, channels: (current.channels || []).some((item) => item.id === channel.id) ? current.channels : [...(current.channels || []), channel] } : current);
    };
    const handleChannelUpdated = ({ serverId: eventServerId, channel } = {}) => {
      if (eventServerId !== serverId || !channel?.id) return;
      setServer((current) => current ? { ...current, channels: (current.channels || []).map((item) => item.id === channel.id ? channel : item) } : current);
    };
    const handleChannelDeleted = ({ serverId: eventServerId, channelId } = {}) => {
      if (eventServerId !== serverId || !channelId) return;
    ...
    return () => {
      ...
      serverSocket.off("connect", watchServer);
  ...
  async function confirmDeleteServer() {
    if (normalizeServerName(deleteName) !== normalizeServerName(server?.name)) {
      setDeleteError("Digite o nome atual do servidor para confirmar.");
      return;
    }
    setServerActionBusy(true);
    setDeleteError("");
  ...
  async function submitCreateChannel(event) {
    ...
    if (!["text", "voice"].includes(channelType)) {
      setChannelError("Escolha um tipo de canal válido.");
      return;
    }
  ...
  async function saveChannelName(channel) {
    ...
    try {
      ...
      if (!data.channel?.id) throw new Error("O servidor não confirmou a alteração do canal.");
  ...
  function removeChannel(channel) {
    if (channel?.isDefault) {
      setSettingsError("O canal padrão não pode ser excluído.");
      return;
    }
    setSettingsError("");
  ...
  async function confirmDeleteChannel() {
    ...
    if (!channel?.id || channelRequestBusy) return;
    setChannelRequestBusy(true);
    setSettingsError("");
    try {
  ...
  function beginRenameChannel(channel) {
  ...
  function toggleProfileUser(profile, anchorRect) {
    if (!profile) return;
    const profileKey = profile.id || profile.userId || profile.socketId || profile.username;
    setProfileUser((current) => {
      const currentProfile = current?.user;
      const currentKey = currentProfile?.id || currentProfile?.userId || currentProfile?.socketId || currentProfile?.username;
      return currentKey && currentKey === profileKey ? null : { user: profile, anchorRect };
    });
  }
  ...
  useEffect(() => {
    if (!createOpen && !settingsOpen && !inviteOpen && !leaveOpen && !deleteOpen && !channelOpen && !channelDeleteTarget) return undefined;
    const handleKeyDown = (event) => {
      if (event.key !== "Escape" || creating || serverActionBusy || channelRequestBusy) return;
      setCreateOpen(false);
      setSettingsOpen(false);
      setInviteOpen(false);
      setLeaveOpen(false);
  ...
  }, [channelDeleteTarget, channelOpen, channelRequestBusy, createOpen, creating, deleteOpen, inviteOpen, leaveOpen, serverActionBusy, settingsOpen]);
  ...
  async function sendMessage(event) {
    ...
    try {
      ...
      playUiSound("message-send", uiNotificationSoundsEnabled(user?.status));
      setDraft("");
      stopServerTyping();
      setSelectedFile(null);
      setReplyingTo(null);
  ...
  useEffect(() => {
    if (serverVoice.isScreenSharing) setIsPipDismissed(false);
  ...
  function renderServerMessage(message, index) {
    ...
    return (
      <article id={`server-message-${message.id}`} className={`fluxer-message-row${isGrouped ? " is-grouped" : ""}`} key={message.id} data-flx="channel.message-group.message" data-message-id={message.id} data-is-group-start={!isGrouped}>
        <div className="fluxer-message-row-grid" data-flx="channel.message-group.message-grid">
          {isGrouped ? <div className="fluxer-message-avatar-placeholder fluxer-message-time" aria-hidden="true"><time dateTime={message.createdAt}>{formatServerTime(message.createdAt)}</time></div> : <button type="button" className="fluxer-message-avatar" onClick={(event) => toggleProfileUser(message.sender, event.currentTarget.getBoundingClientRect())} aria-label={`Ver perfil de ${senderName}`}><UserAvatar user={message.sender} size={36} /></button>}
          <div className="fluxer-message-body" data-flx="channel.message-group.message-body">
          {!isGrouped && <div className="fluxer-message-meta"><strong>{senderName}</strong><time dateTime={message.createdAt} title={formatServerDateTime(message.createdAt)}>{formatServerTime(message.createdAt)}</time></div>}
          ...
          {attachment && attachmentSource && <div className="fluxer-message-attachment" data-flx="channel.message-attachments">
            {attachment.type === "image" ? <button type="button" className="fluxer-message-image-button" onClick={() => setLightboxImage({ source: attachmentSource, alt: attachment.name || "Imagem anexada" })}><img src={attachmentSource} alt={attachment.name || "Imagem anexada"} onError={handleServerImageError} /></button> : attachment.type === "video" ? <video controls preload="metadata" src={attachmentSource} /> : <a className="fluxer-file-attachment" href={attachmentSource} target="_blank" rel="noreferrer" download>{attachment.name || "Arquivo"}</a>}
  ...
  return <main className={`page app-shell fluxer-server-shell ${activeContentView === "media" && !isMemberPanelOpen ? "call-members-collapsed" : ""}`} data-flx="layout.server-shell">
    ...
    <section className="central-stage">
      ...
      {activeChannel?.type === "text" && <ChatComposerFrame onSubmit={sendMessage}>
        ...
        <ChatComposerRow>
          <button type="button" className="fluxer-composer-attach" onClick={() => { setIsAttachMenuOpen((current) => !current); setIsEmojiPickerOpen(false); }} disabled={isSending} title="Adicionar anexo" aria-label="Adicionar anexo" aria-haspopup="menu" aria-expanded={isAttachMenuOpen}><Icon name="plus" size={17} /></button>
          {isAttachMenuOpen && <div className="fluxer-composer-popover fluxer-composer-attach-menu" role="menu" aria-label="Adicionar anexo"><button type="button" role="menuitem" onClick={() => openServerFilePicker(MEDIA_ACCEPT)}><Icon name="image" size={15} /><span>Enviar imagem ou vídeo</span></button><button type="button" role="menuitem" onClick={() => openServerFilePicker(FILE_ACCEPT)}><Icon name="file" size={15} /><span>Enviar arquivo</span></button></div>}
          <input ref={fileInputRef} className="visually-hidden" type="file" accept={fileAccept} onChange={handleServerFileChange} />
          <textarea ref={messageInputRef} className="fluxer-composer-input" value={draft} onChange={(event) => handleDraftChange(event.target.value)} onKeyDown={handleMessageKeyDown} placeholder={`Mensagem em #${activeChannel.name}`} maxLength={4000} rows={1} disabled={isSending} aria-label={`Mensagem para #${activeChannel.name}`} />
          <div className="fluxer-composer-actions"><button type="button" className="fluxer-composer-button" onClick={() => { setIsEmojiPickerOpen((current) => !current); setIsAttachMenuOpen(false); }} disabled={isSending} title="Inserir emoji" aria-label="Inserir emoji" aria-expanded={isEmojiPickerOpen}><Icon name="smile" size={16} /></button>{(draft.trim() || selectedFile) && <button type="submit" className="fluxer-composer-send" disabled={isSending} aria-label={isSending ? "Enviando" : "Enviar mensagem"}><Icon name="send" size={15} /></button>}</div>
          {isEmojiPickerOpen && <div className="fluxer-composer-popover"><EmojiPicker onSelect={insertServerEmoji} /></div>}
        </ChatComposerRow>
      </ChatComposerFrame>}
      </div>
    </section>}
    </section>
    <ParticipantsPanel heading="Membros" participants={memberParticipants} showMedia={false} showPresenceIndicator={false} onProfileClick={(person, anchorRect) => toggleProfileUser(person || user, anchorRect)} onParticipantClick={toggleProfileUser} />
    {profileUser && profileUser.user?.id === user?.id && <ProfilePopover accountUser={user} profile={{ ...profileUser.user, status: user?.status }} nickname={profileUser.user.displayName || user?.displayName} avatarUrl={user?.avatarUrl || ""} isInVoice={serverVoice.connected} voiceChannelName={voiceChannels.find((channel) => channel.id === voiceChannelId)?.name || "Geral"} onStatusChange={changeAccountStatus} onEditProfile={() => { setProfileUser(null); onOpenAccountSettings?.("profile"); }} onOpenSettings={() => { setProfileUser(null); onOpenAccountSettings?.("profile"); }} onLogout={async () => { await logout(); onNavigateHome?.(); }} onClose={() => setProfileUser(null)} />}
    {profileUser && profileUser.user?.id !== user?.id && <SocialUserProfilePopover participant={profileUser.user} anchorRect={profileUser.anchorRect} onClose={() => setProfileUser(null)} onViewProfile={(person) => { setProfileUser(null); setProfileDetailsUser(person); }} />}
      ...
      <section className="server-settings-modal" role="dialog" aria-modal="true" aria-labelledby="server-settings-title">
        ...
        <div className="server-settings-layout">
          <nav className="server-settings-nav" aria-label="Secoes do servidor">
            ...
            {canManageServer && <button type="button" className={settingsSection === "members" ? "is-active" : ""} onClick={() => setSettingsSection("members")}><Icon name="members" size={16} />Membros</button>}
            {canManageServer && <button type="button" className={settingsSection === "invites" ? "is-active" : ""} onClick={() => setSettingsSection("invites")}><Icon name="link" size={16} />Convites</button>}
            <span />
          ...
          <form id="server-settings-form" className="server-settings-content" onSubmit={submitServerSettings}>
            ...
            {settingsSection === "channels" && canManageServer && <section><div className="server-settings-section-row"><div><span className="section-label">ORGANIZACAO</span><h3>Canais</h3></div><button type="button" className="secondary-button" onClick={() => { setSettingsOpen(false); openCreateChannel("text"); }}><Icon name="plus" size={15} />Novo canal</button></div><p>Renomeie ou remova canais sem sair deste contexto.</p><div className="server-settings-channel-list">{(server?.channels || []).map((channel) => <div className="server-settings-channel-row" key={channel.id}><Icon name={channel.type === "voice" ? "voice" : "hash"} size={16} />{editingChannelId === channel.id ? <input className="text-input" value={editingChannelName} onChange={(event) => setEditingChannelName(event.target.value)} maxLength={40} autoFocus /> : <span><strong>{channel.name}</strong><small>{channel.type === "voice" ? "Voz" : "Texto"}{channel.isDefault ? " · Canal padrão" : ""}</small></span>}<div>{editingChannelId === channel.id ? <><button type="button" className="icon-button" onClick={() => saveChannelName(channel)} title="Salvar" aria-label="Salvar nome"><Icon name="check" size={15} /></button><button type="button" className="icon-button" onClick={() => setEditingChannelId("")} title="Cancelar" aria-label="Cancelar"><Icon name="close" size={15} /></button></> : <><button type="button" className="icon-button" onClick={() => { setEditingChannelId(channel.id); setEditingChannelName(channel.name); }} title="Renomear" aria-label={`Renomear ${channel.name}`}><Icon name="edit" size={15} /></button><button type="button" className={`icon-button ${channel.isDefault ? "is-disabled" : "danger-menu-item"}`} onClick={() => removeChannel(channel)} disabled={channel.isDefault} title={channel.isDefault ? "O canal padrão não pode ser excluído" : "Excluir"} aria-label={channel.isDefault ? "Canal padrão não pode ser excluído" : `Excluir ${channel.name}`}><Icon name="trash" size={15} /></button></>}</div></div>)}</div></section>}
            {settingsSection === "members" && canManageServer && <section><span className="section-label">PESSOAS</span><h3>Membros</h3><p>Veja quem participa e qual é a função de cada pessoa neste servidor.</p><div className="server-settings-member-list">{members.map((member) => <div className="server-settings-member-row" key={member.id}><UserAvatar user={member} size={32} /><span><strong>{member.displayName || member.username}</strong><small>@{member.username}</small></span><small>{member.role === "owner" ? "Proprietário" : member.role === "admin" ? "Administrador" : "Membro"}</small></div>)}</div></section>}
            {settingsSection === "invites" && canManageServer && <section><span className="section-label">ACESSO</span><h3>Convites</h3><p>Crie um link para convidar pessoas. A entrada continua protegida pelas permissões do servidor.</p><button type="button" className="secondary-button" onClick={() => { setSettingsOpen(false); openServerInvite(); }}><Icon name="link" size={15} />Criar convite</button></section>}
    ...
    {deleteOpen && <div className="modal-backdrop server-create-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !serverActionBusy) setDeleteOpen(false); }}><section className="server-create-modal" role="dialog" aria-modal="true" aria-labelledby="server-delete-title"><header><div><span className="section-label">AÇÃO PERMANENTE</span><h2 id="server-delete-title">Excluir servidor?</h2></div><button type="button" className="icon-button" onClick={() => setDeleteOpen(false)} disabled={serverActionBusy} aria-label="Fechar"><Icon name="close" size={16} /></button></header><p>Você está prestes a excluir permanentemente: <strong>{server?.name}</strong>. Canais, mensagens, membros, convites e configurações serão removidos. Esta ação não pode ser desfeita.</p><form id="server-delete-form" onSubmit={(event) => { event.preventDefault(); confirmDeleteServer(); }}><label className="field-label" htmlFor="server-delete-name">Digite o nome do servidor para confirmar:</label><input id="server-delete-name" className="text-input" value={deleteName} onChange={(event) => { setDeleteName(event.target.value); setDeleteError(""); }} autoFocus aria-invalid={Boolean(deleteError)} />{deleteError && <small className="field-error">{deleteError}</small>}</form><footer><button type="button" className="secondary-button" onClick={() => setDeleteOpen(false)} disabled={serverActionBusy}>Cancelar</button><button type="submit" form="server-delete-form" className="primary-button danger-action-button" disabled={serverActionBusy || normalizeServerName(deleteName) !== normalizeServerName(server?.name)}>{serverActionBusy ? "Excluindo..." : "Excluir servidor"}</button></footer></section></div>}
    {channelOpen && <div className="modal-backdrop server-create-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !channelRequestBusy) setChannelOpen(false); }}><section className="server-create-modal server-channel-modal" role="dialog" aria-modal="true" aria-labelledby="server-channel-title"><header><div><span className="section-label">CANAIS</span><h2 id="server-channel-title">Criar canal</h2></div><button type="button" className="icon-button" onClick={() => setChannelOpen(false)} disabled={channelRequestBusy} aria-label="Fechar"><Icon name="close" size={16} /></button></header><form id="server-channel-form" onSubmit={submitCreateChannel}><fieldset className="server-channel-type"><legend>Tipo de canal</legend>{[["text", "hash", "Texto", "Envie mensagens, imagens e arquivos."], ["voice", "voice", "Voz", "Converse por voz, camera e compartilhamento."]].map(([value, icon, title, description]) => <label className={channelType === value ? "is-selected" : ""} key={value}><input type="radio" name="server-channel-type" value={value} checked={channelType === value} onChange={(event) => setChannelType(event.target.value)} /><span className="server-channel-type-icon"><Icon name={icon} size={18} /></span><span><strong>{title}</strong><small>{description}</small></span><i aria-hidden="true" /></label>)}</fieldset><label className="field-label" htmlFor="server-channel-name">Nome do canal</label><input id="server-channel-name" className="text-input" value={channelName} onChange={(event) => { setChannelName(event.target.value); setChannelError(""); }} maxLength={40} autoFocus aria-invalid={Boolean(channelError)} placeholder={channelType === "voice" ? "novo-canal-de-voz" : "novo-canal"} />{channelError && <small className="field-error" role="alert">{channelError}</small>}</form><footer><button type="button" className="secondary-button" onClick={() => setChannelOpen(false)} disabled={channelRequestBusy}>Cancelar</button><button type="submit" form="server-channel-form" className="primary-button" disabled={channelRequestBusy || !channelName.trim()}>{channelRequestBusy ? "Criando..." : "Criar canal"}</button></footer></section></div>}
    {channelDeleteTarget && <div className="modal-backdrop server-create-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !channelRequestBusy) setChannelDeleteTarget(null); }}><section className="server-create-modal server-channel-delete-modal" role="dialog" aria-modal="true" aria-labelledby="server-channel-delete-title"><header><div><span className="section-label">AÇÃO PERMANENTE</span><h2 id="server-channel-delete-title">Excluir canal?</h2></div><button type="button" className="icon-button" onClick={() => setChannelDeleteTarget(null)} disabled={channelRequestBusy} aria-label="Fechar"><Icon name="close" size={16} /></button></header><p>O canal <strong>{channelDeleteTarget.name}</strong> e suas mensagens serão removidos permanentemente.</p>{settingsError && <small className="field-error" role="alert">{settingsError}</small>}<footer><button type="button" className="secondary-button" onClick={() => setChannelDeleteTarget(null)} disabled={channelRequestBusy}>Cancelar</button><button type="button" className="primary-button danger-action-button" onClick={confirmDeleteChannel} disabled={channelRequestBusy}>{channelRequestBusy ? "Excluindo..." : "Excluir canal"}</button></footer></section></div>}
