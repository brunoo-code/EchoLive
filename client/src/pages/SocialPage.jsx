...
function AuthenticatedSocialPage({ user, onNavigateHome, onNavigateDm, onOpenAccountSettings }) {
  ...
  return <main className="page social-page">
    ...
    <section className="social-content">
      <header className="social-topbar"><div className="social-topbar-title"><Icon name="account" size={18} /><strong>Amigos</strong></div><nav className="social-tabs" aria-label="Visoes de Amigos">{[["online", "Online"], ["all", "Todos"], ["pending", "Pendentes"]].map(([id, label]) => <button type="button" key={id} className={activeTab === id ? "is-active" : ""} onClick={() => changeTab(id)}>{label}{id === "pending" && pendingCount > 0 && <b className="social-tab-badge">{pendingCount > 9 ? "9+" : pendingCount}</b>}</button>)}<button type="button" className={`social-tab-add ${activeTab === "add" ? "is-active" : ""}`} onClick={() => changeTab("add")}><Icon name="plus" size={14} />Adicionar amigo</button></nav></header>
...
function FriendRow({ item, isOnline, onMessage, onOpenProfile, removeTarget, setRemoveTarget, onRemove }) {
  ...
  return <div className="social-person-row" data-flx="channel.friends.friend-list-item"><button type="button" className="social-person-identity" onClick={(event) => onOpenProfile(item.user, event.currentTarget.getBoundingClientRect())}><span className="social-person-avatar"><Avatar user={item.user} size={36} /><StatusDot status={visiblePresence === "online" ? "online" : visiblePresence === "dnd" ? "dnd" : "offline"} /></span><span className="social-person-copy"><strong>{item.user.displayName || item.user.username}</strong><small>{presenceLabel}</small></span></button><span className="social-row-actions"><IconButton className="social-message-action" label={`Enviar mensagem para ${item.user.displayName || item.user.username}`} onClick={() => onMessage(item.user.id)}><Icon name="chat" size={15} /></IconButton><IconButton label={`Mais acoes para ${item.user.username}`} onClick={() => setRemoveTarget(removeTarget === item.user.id ? null : item.user.id)}><Icon name="more" size={16} /></IconButton></span>{removeTarget === item.user.id && <span className="social-remove-confirm"><span>Remover amizade?</span><button type="button" className="text-button danger" onClick={() => onRemove(item.user.id)}>Remover</button><button type="button" className="text-button" onClick={() => setRemoveTarget(null)}>Cancelar</button></span>}</div>;
}
